...
const NL_TOOLS: Anthropic.Tool[] = [
  ...
  {
    name: "clarify",
    description: "문장이 위 항목 중 어디에도 명확히 해당하지 않거나, 저장에 꼭 필요한 정보(학생 이름 등)가 빠져 있을 때 사용한다. 절대로 정보를 추측해서 지어내지 않는다.",
    input_schema: {
      type: "object",
      properties: {
...
- 학생 이름은 아래 재원생 명단에 있는 이름과 최대한 정확히 일치시킨다. 명단에 없는 이름이면 문장에 적힌 그대로 사용한다.
...
- 저장에 꼭 필요한 정보(학생 이름 등)가 빠져 있거나 어느 항목에도 명확히 해당하지 않으면 반드시 clarify 도구를 사용한다. 추측해서 지어내지 않는다.
