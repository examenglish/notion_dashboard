...
function stableUuid(v){const b=Buffer.from(createHash('sha256').update(`fixture:${v}`).digest().subarray(0,16));b[6]=(b[6]&15)|80;b[8]=(b[8]&63)|128;const h=b.toString('hex');return`${h.slice(0,8)}-${h.slice(8,12)}-${h.slice(12,16)}-${h.slice(16,20)}-${h.slice(20)}`;}
...
const targets={CLASS:{'소속학생':'STUDENT','담당조교':'STAFF'},STUDENT:{'소속반':'CLASS'},CLASS_PROGRESS:{'반':'CLASS','생성된학생기록':'DAILY_RECORD'},DAILY_RECORD:{'학생':'STUDENT','반':'CLASS','반별진도원본':'CLASS_PROGRESS'},BRIEFING:{'학생':'STUDENT'},EXAM_SCORE:{'학생':'STUDENT'},COUNSELING:{'학생':'STUDENT'},ADMIN_INBOX:{'대상학생':'STUDENT'},TODO:{'담당자':'STAFF','관련학생':'STUDENT','관련반':'CLASS','클리닉보고':'CLINIC','상위업무':'TODO'},CLINIC:{'조교':'STAFF','담당학생':'STUDENT','담당강사':'STAFF','관련업무':'TODO'},MATERIAL:{'요청자':'STAFF','담당자':'STAFF'},EXAM_PREP:{'학생':'STUDENT'},SLACK_RECORDS:{'학생':'STUDENT'},MANUAL_STEP:{'매뉴얼':'MANUAL'}};
