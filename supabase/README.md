...
- **감사 이후 drift**: 원본 감사(위 commit 기준)는 당시 `lib/notion.ts` 3873행 스냅샷을 근거로 했다. 현재 이 저장소의 `lib/notion.ts`는 4414행이며, TODO/STAFF에 신규 property가 추가되었고 MANUAL/MANUAL_STEP 두 개의 신규 Notion DB(화면녹화 AI 매뉴얼 기능)가 생겼다. 전체 내역은 [`docs/DRIFT_ANALYSIS.md`](docs/DRIFT_ANALYSIS.md) 참고. **아직 schema/script에 반영되지 않았다** — 다음 작업(Phase 1)은 이 drift를 `schema/001_initial_schema.sql`과 `scripts/migrate_notion_to_supabase.mjs`에 반영하는 것이다.
